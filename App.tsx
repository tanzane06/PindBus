...
import { HomeIcon, RouteIcon, UserIcon, SmsIcon, LocationMarkerIcon, SignalIcon, SearchIcon, BusIcon, StarIcon, PunjabMapIcon, BellIcon, GlobeAltIcon, WifiOffIcon, MoonIcon, PindBusLogo, BusSimpleIcon, SwapVerticalIcon } from './components/Icons';
...
const DamaskPattern = () => (
    <svg className="absolute inset-0 w-full h-full opacity-[0.03]" xmlns="http://www.w3.org/2000/svg" width="80" height="80" viewBox="0 0 80 80">
      <path d="M40 0C17.9 0 0 17.9 0 40s17.9 40 40 40 40-17.9 40-40S62.1 0 40 0zM25.7 60.3c-2.4-1.4-4.8-2.9-7-4.6 3.6-6.1 7.2-12.2 10.8-18.4 2.1 1.2 4.1 2.4 6.2 3.6-4.6 7.8-9.1 15.5-13.7 23.3-2.2-1.3-4.4-2.6-6.6-3.9 3.6-6.1 7.2-12.2 10.8-18.4 2.1 1.2 4.1 2.4 6.2 3.6-4.6 7.8-9.1 15.5-13.7 23.3zM40 80c-2.8 0-5.6-.4-8.3-1.1 1.6-2.7 3.2-5.5 4.8-8.2 2.3 1.3 4.6 2.6 6.9 3.9C42.4 77.2 41.2 78.7 40 80zm14.3-19.7c-3.6 6.1-7.2 12.2-10.8 18.4-2.1-1.2-4.1-2.4-6.2-3.6 4.6-7.8 9.1-15.5 13.7-23.3 2.2 1.3 4.4 2.6 6.6 3.9-3.6 6.1-7.2 12.2-10.8 18.4-2.1-1.2-4.1-2.4-6.2-3.6 4.6-7.8 9.1-15.5 13.7-23.3z" fill="currentColor"/>
    </svg>
);
...
type FoundRoute = Route & { price: number; nextBusEta: number | null };

const HomeScreen: FC<{
    boardingStop: Stop | null;
    destinationStop: Stop | null;
    onSelectBoarding: () => void;
    onSelectDestination: () => void;
    onSwap: () => void;
    onViewLiveMap: () => void;
    foundRoutes: FoundRoute[] | null;
    onViewRouteDetails: (routeId: string) => void;
    t: (key: string, replacements?: { [key: string]: string | number }) => string;
}> = ({ boardingStop, destinationStop, onSelectBoarding, onSelectDestination, onSwap, onViewLiveMap, foundRoutes, onViewRouteDetails, t }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-[#f0f4f8] dark:bg-gray-900">
            {/* Top Section */}
            <div className="relative bg-[#0d2a4c] rounded-b-3xl shadow-lg px-6 pt-12 pb-24 text-white overflow-hidden">
                <DamaskPattern />
                <div className="relative z-10">
                    <h1 className="font-serif text-5xl font-bold">{t('home_welcome')}</h1>
                    <p className="opacity-80 mt-1">{t('home_heading')}</p>
                </div>
            </div>

            {/* Main Content pushed up into the top section */}
            <main className="flex-grow flex flex-col px-6 -mt-16">
                {/* Search Card */}
                <div className="relative bg-[#e0f0f8] dark:bg-gray-700/80 backdrop-blur-sm p-5 rounded-2xl shadow-lg border border-white/10 flex-shrink-0">
                    <div className="flex items-center">
                        <div className="flex-grow space-y-4">
                            <button onClick={onSelectBoarding} className="flex items-center space-x-3 w-full text-left">
                                <BusSimpleIcon className="w-6 h-6 text-[#0d2a4c] dark:text-sky-300"/>
                                <span className={`font-semibold text-lg truncate ${boardingStop ? 'text-gray-800 dark:text-stone-200' : 'text-gray-500 dark:text-stone-400'}`}>
                                    {boardingStop ? boardingStop.name : t('home_from')}
                                </span>
                            </button>
                            <hr className="border-t border-gray-400/50 ml-9"/>
                            <button onClick={onSelectDestination} className="flex items-center space-x-3 w-full text-left">
                                <BusSimpleIcon className="w-6 h-6 text-[#0d2a4c] dark:text-sky-300"/>
                                <span className={`font-semibold text-lg truncate ${destinationStop ? 'text-gray-800 dark:text-stone-200' : 'text-gray-500 dark:text-stone-400'}`}>
                                    {destinationStop ? destinationStop.name : t('home_to')}
                                </span>
                            </button>
                        </div>
                        <button onClick={onSwap} className="p-2 bg-white dark:bg-gray-600 rounded-full shadow-md ml-4 hover:bg-gray-100 transition">
                            <SwapVerticalIcon className="w-6 h-6 text-[#0d2a4c] dark:text-sky-300"/>
                        </button>
                    </div>
                    ...
                    <div className="flex justify-between items-end">
                        <div>
                            <p className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-2">{t('calendar')}</p>
                            <div className="flex space-x-2 sm:space-x-3">
                                {calendarDays.map(d => (
                                    <button key={d.date} onClick={() => setSelectedDate(d.date)} className="flex flex-col items-center p-1 rounded-md transition hover:bg-black/5">
                                        <div className={`w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center rounded-full font-bold transition-colors ${selectedDate === d.date ? 'bg-[#0d2a4c] text-white shadow-md' : 'bg-transparent text-gray-700 dark:text-stone-300'}`}>
                                            {d.date}
                                        </div>
                                        <span className="text-xs mt-1 text-gray-500 dark:text-gray-400">{d.day}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
                        <div className="text-right flex-shrink-0 ml-2">
                            <p className="font-semibold text-gray-700 dark:text-stone-300">September</p>
                ...
                <button
                    ...
                    className="relative w-full mt-4 bg-[#0d2a4c] text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center space-x-3 overflow-hidden transition-transform transform hover:scale-105"
                >
                    <DamaskPattern />
                    <LocationMarkerIcon className="w-6 h-6 z-10"/>
                    <span className="z-10 text-lg">{t('home_view_map')}</span>
                </button>

                {/* Dynamic Content Area */}
                <div className="flex-grow mt-4 flex flex-col min-h-0">
                    {foundRoutes === null ? (
                        <div className="text-center py-8 my-auto flex flex-col items-center justify-center h-full">
                            <SearchIcon className="w-16 h-16 text-gray-300 dark:text-gray-600 mx-auto" />
                            <h3 className="mt-2 text-lg font-semibold text-gray-700 dark:text-stone-300">{t('home_find_your_route')}</h3>
                            <p className="text-sm text-gray-500 dark:text-stone-400 max-w-xs">{t('home_find_your_route_desc')}</p>
                        </div>
                    ) : foundRoutes.length > 0 ? (
                        <div className="overflow-y-auto space-y-3 pb-4">
                             <h2 className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-2 px-1">{t('available_routes')}</h2>
                             {foundRoutes.map(route => (
                                <button key={route.id} onClick={() => onViewRouteDetails(route.id)} className="w-full text-left bg-white dark:bg-gray-700 p-3 rounded-lg shadow-md hover:shadow-lg transition-shadow flex items-center space-x-4 border border-stone-200 dark:border-gray-600">
                                    <div className="w-12 h-12 rounded-lg flex items-center justify-center text-white font-bold text-lg flex-shrink-0" style={{ backgroundColor: route.color }}>
                                        {route.id.split('-')[1].toUpperCase()}
                                    </div>
                                    <div className="flex-grow">
                                        <p className="font-bold text-gray-800 dark:text-stone-200">{route.name}</p>
                                        {route.nextBusEta !== null ? (
                                             <p className="text-sm text-emerald-600 dark:text-emerald-400 font-semibold">{t('next_bus_eta', { eta: route.nextBusEta })}</p>
                                        ) : (
                                            <p className="text-sm text-stone-500 dark:text-stone-400">{t('no_live_bus_data')}</p>
                                        )}
                                    </div>
                                    <div className="text-right flex-shrink-0">
                                        <p className="text-xl font-bold text-gray-800 dark:text-stone-200">{t('rupee_symbol')}{route.price}</p>
                                        <p className="text-xs text-stone-500 dark:text-stone-400">{t('ticket_price')}</p>
                                    </div>
                                </button>
                             ))}
                        </div>
                        ...
                        <div className="text-center py-8 my-auto flex flex-col items-center justify-center h-full">
                            <BusIcon className="w-16 h-16 text-gray-300 dark:text-gray-600 mx-auto" />
                            <h3 className="mt-2 text-lg font-semibold text-gray-700 dark:text-stone-300">{t('home_no_route_found')}</h3>
                            <p className="text-sm text-gray-500 dark:text-stone-400 max-w-xs">{t('home_no_route_desc')}</p>
                        </div>
                    )}
                </div>
            </main>
...
const RoutesScreen: FC<{ 
    ...
    homeBoardingStop: Stop | null;
    homeDestinationStop: Stop | null;
}> = ({ routes, buses, getEtaMinutes, getTicketPrice, selectedRouteId, setSelectedRouteId, setActiveScreen, t, toggleFavourite, isFavourite, homeBoardingStop, homeDestinationStop }) => {
    ...
    useEffect(() => {
        if (selectedRouteId) {
            ...
            if (newRoute && newRoute.id !== viewingRoute?.id) {
                setViewingRoute(newRoute);

                const boardOnRoute = homeBoardingStop && newRoute.stops.some(s => s.id === homeBoardingStop.id);
                const destOnRoute = homeDestinationStop && newRoute.stops.some(s => s.id === homeDestinationStop.id);

                if (boardOnRoute && destOnRoute) {
                    setBoardingStop(homeBoardingStop);
                    setDestinationStop(homeDestinationStop);
                } else {
    ...
    }, [selectedRouteId, routes, viewingRoute, homeBoardingStop, homeDestinationStop]);
...
const BottomNav: FC<{ activeScreen: Screen; setActiveScreen: (screen: Screen) => void; t: (key: string) => string }> = ({ activeScreen, setActiveScreen, t }) => {
  ...
  return (
    <nav className="relative flex justify-around items-center bg-[#0d2a4c] h-20 overflow-hidden shadow-[0_-2px_10px_rgba(0,0,0,0.1)]">
        <DamaskPattern />
      {navItems.map(item => (
        <button
          key={item.id}
          onClick={() => setActiveScreen(item.id as Screen)}
          className={`relative z-10 flex flex-col items-center justify-center w-full h-full transition-colors ${activeScreen === item.id ? 'text-white' : 'text-gray-400 hover:text-white'}`}
        >
        ...
        case 'HOME':
            return props.isLowBandwidthMode ? (
                ...
                <HomeScreen
                    ...
                    onSelectBoarding={props.onSelectBoarding}
                    onSelectDestination={props.onSelectDestination}
                    onSwap={props.onSwap}
                    onViewLiveMap={props.onViewLiveMap}
                    foundRoutes={props.foundRoutes}
                    onViewRouteDetails={props.onViewRouteDetails}
                    t={props.t}
                />
            );
        case 'ROUTES':
            return <RoutesScreen 
                        ...
                        homeBoardingStop={props.boardingStop}
                        homeDestinationStop={props.destinationStop}
                    />;
        case 'SMS':
...
const App: FC = () => {
    ...
    const [boardingStop, setBoardingStop] = useState<Stop | null>(null);
    const [destinationStop, setDestinationStop] = useState<Stop | null>(null);
    const [foundRoutes, setFoundRoutes] = useState<FoundRoute[] | null>(null);
    ...
    useEffect(() => {
        if (boardingStop && destinationStop) {
            const matchingRoutes = routes.map(route => {
                const startIndex = route.stops.findIndex(s => s.id === boardingStop.id);
                const endIndex = route.stops.findIndex(s => s.id === destinationStop.id);
                
                if (startIndex !== -1 && endIndex !== -1) {
                    const price = getTicketPrice(route, boardingStop, destinationStop);
                    const busOnRoute = buses.find(b => b.routeId === route.id);
                    
                    let nextBusEta: number | null = null;
                    if (busOnRoute) {
                        const busIsCorrectDirection = (startIndex < endIndex && busOnRoute.direction === 1) || (startIndex > endIndex && busOnRoute.direction === -1);
                        
                        if (busIsCorrectDirection) {
                            const eta = getEtaMinutes(busOnRoute, startIndex);
                            if (eta >= 0) {
                               nextBusEta = eta;
                            }
                        }
                    }
                    return { ...route, price, nextBusEta };
                }
                return null;
    ...
    }, [boardingStop, destinationStop, routes, buses, getTicketPrice, getEtaMinutes]);
    ...
    const handleSwapStops = () => {
        const temp = boardingStop;
        setBoardingStop(destinationStop);
        ...
        <div className="h-screen w-screen max-w-md mx-auto flex flex-col font-sans bg-stone-100 dark:bg-gray-800 shadow-2xl">
            <main className="flex-grow overflow-hidden relative">
                <ScreenRenderer
                    ...
                    onSelectBoarding={() => setStopSelector({ visible: true, type: 'boarding' })}
                    onSelectDestination={() => setStopSelector({ visible: true, type: 'destination' })}
                    onSwap={handleSwapStops}
                    onViewLiveMap={handleViewLiveMap}
